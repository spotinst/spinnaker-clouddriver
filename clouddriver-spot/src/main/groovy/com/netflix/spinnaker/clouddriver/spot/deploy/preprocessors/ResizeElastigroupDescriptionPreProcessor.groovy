...
@Component
class ResizeElastigroupDescriptionPreProcessor implements AtomicOperationDescriptionPreProcessor {
  @Override
  boolean supports(Class descriptionClass) {
    return descriptionClass == ResizeElastigroupDescription
  }
  ...
  Map process(Map description) {
    description.with {
      ...
      if (!elastigroups) {
        elastigroups = [
          [serverGroupName: serverGroupName ?: elastigroupName, region: region, capacity: capacity, elastigroupId: elastigroupId]
        ]
      }
